...
public final class RollingFileAppender extends FileAppender {
  ...
  /**
   * Lock to protect the length.
   */
  private Object fileLengthLock = new Object();
      ...
      try {
        ...
        if (rollover != null) {
          ...
          if (lastRolloverAsyncAction != null) {
            Thread t = new Thread(lastRolloverAsyncAction);
			t.setName("log4j-rollover-" + getName());
            t.start();
          }
        }
        ...
        if (getAppend()) {
          setFileLength(activeFile.length());
        } else {
		  setFileLength(0);
        }
            ...
            if (rollover.getActiveFileName().equals(getFile())) {
              ...
              if (success) {
                if (rollover.getAppend()) {
                  setFileLength(new File(rollover.getActiveFileName()).length());
                } else {
                  setFileLength(0);
                }
  ...
  private boolean isTriggeringEvent(LoggingEvent event) {
    return triggeringPolicy.isTriggeringEvent(
  ...
  */
  ...
  public long getFileLength() {
	synchronized (fileLengthLock) {
      return fileLength;
	}
  }

  private void setFileLength(long len) {
	synchronized (fileLengthLock) {
      fileLength = len;
	}
  }
