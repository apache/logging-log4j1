  ...
  */
  ...
  public int canContainComponent(String name) {
    ...
    if(propertyDescriptor != null) {
      ...
      if (setter != null) {
	logger.debug("Found setter method for property ["+name+"] in class "+objClass.getName());
	return AS_PROPERTY;
      }
    }     
    // we have failed
  ...
  public void setComponent(String name, Object childComponent) {
    ...
    Class[] paramTypes = setter.getParameterTypes();
    if (paramTypes.length != 1) {
      logger.error("Wrong number of parameters in setter method for property ["+name+"] in "+
		  obj.getClass().getName());
       return;
    }
	...
	logger.debug("Set child component of type [" + childComponent.getClass().getName() 
